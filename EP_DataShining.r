...
data.ep.responseTest[timeCount%in%c(2000:1800)&testType=="Grad"&mvOpening=="MV=0.75",#&&testVset<=20,
                            c("timeCount","testId","Valveopening","Vset","Flowrate",
                              "InWindT","t_out_set","isOn","Subpressure",
                              "OutWindT","t_return_set","predPressureStatus",
  ...
  melt(.,id.var=c("timeCount","testId","isOn","predPressureStatus"))%>%#,"Ti","para","testVset"
  as.data.table(.)%>%{ #,"InWindT","t_out_set","resistanceS"
    ggplot(data=.[variable %in% c("Valveopening","Vset")],aes(x=timeCount,y=value,color=variable,width=4,group=paste(testId,variable)))+
      ...
      geom_point(data=.[variable %in% c("Subpressure")],aes(x=timeCount,y=value,shape=predPressureStatus))+
      geom_line(data=.[variable %in% c("Subpressure")],aes(x=timeCount,y=value))+#)+value#"OutWindT",(value-20)*5)
