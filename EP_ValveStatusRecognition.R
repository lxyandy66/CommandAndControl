...
#数据趋势用可视化
data.ep.realTest[timeCount<200,
    c("testId","timeCount","Subpressure","predSubPresStatus","Valveopening","Subpressure","Totalpressure","predTotPresStatus")]%>%
  melt(.,id.var=c("testId","timeCount","predSubPresStatus","predTotPresStatus"))%>%{#,"flowrateStatus","fiveFlowrateMva","flowrateStatus","predTotPresStatus"
    # browser()  
    ggplot(data = .,aes(x=timeCount,y=value,color=variable))+
        geom_line(data = .[variable=="Subpressure"],aes(x=timeCount,y=value))+
        geom_line(data = .[variable=="Totalpressure"],aes(x=timeCount,y=value))+
        geom_point(data = .[variable=="Totalpressure"],aes(x=timeCount,y=value,shape=predTotPresStatus))+
        geom_point(data = .[variable=="Subpressure"],aes(x=timeCount,y=value,shape=predSubPresStatus))+#,shape=flowrateStatus
